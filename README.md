...
| | It's a package maanager for Node.js  |
|Question | What is the `npm install` command and what is the purpose of the `--save` flag?  |
| | It's installing a package, the "--save" put it automatically in dependencies but it's deprecated  |
|Question | How can we use the `https://www.npmjs.com/` web site?  |
| | *Enter your response here...*  |
|Question | In JavaScript, how can we **generate a UUID** compliant with RFC4122? |
| | By using UUID package of Node.js  |
...
| | Create the Dockerfile and then execute "docker build" command |
|Question | How can we use the `ENTRYPOINT` statement in our Dockerfile?  |
| | To pass argument to our application (musician)  |
|Question | After building our Docker image, how do we use it to **run containers**?  |
| | By executing "docker run" command  |
|Question | How do we get the list of all **running containers**?  |
| | docker ps  |
|Question | How do we **stop/kill** one running container?  |
| | docker rm  |
|Question | How can we check that our running containers are effectively sending UDP datagrams?  |
| | By sniffing with Wireshark or Tcpdump  |
...
| | By using the listen() method on a socket object  |
|Question | How can we use the `Map` built-in object introduced in ECMAScript 6 to implement a **dictionary**?  |
| | Create the map with "new Map(Array)" |
|Question | How can we use the `Moment.js` npm module to help us with **date manipulations** and formatting?  |
| | To make diffference between two dates easier (using diff()) |
|Question | When and how do we **get rid of inactive players**?  |
| | When the difference between the actual moment and the moment in the array is greater than 5 seconds |
|Question | How do I implement a **simple TCP server** in Node.js?  |
| | Create a TCP Socket using "net" package and then listen on it and react to event like "data" or stuff like that |
